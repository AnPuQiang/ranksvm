 ...
 * 负 责 人   : an
 * 创建日期   : 2017年5月4日
 * 文件描述   : ranking svm 主要功能函数集合
 * 版权说明   : Copyright (c) 2017-2017   哈工大501实验室
 * 其    他   : 
 * 修改日志   : 
***********************************************************************************/
...
class Kernel_Q
{
	public:
		...
		~Kernel_Q();
		void Qv(double *s, double *Qs);	//计算矩阵乘积
...
/*****************************************************************************
 * 函 数 名  : Kernel_Q.k_function
 * 负 责 人  : an
 * 创建日期  : 2017年5月4日
 * 函数功能  : 计算核函数的值
 * 输入参数  : const svm_node *x           xi
               const svm_node *y           xj
               const svm_parameter& param  核函数类型
 * 输出参数  : 无
 * 返 回 值  : double
 * 调用关系  : 
 * 其    它  : 
...
l2r_rank_fun::l2r_rank_fun(const svm_problem *prob, const svm_parameter *param , double *C): Q(*prob, *param, block_size)
{
	...
	*C = CC/nr_query;	//修改部分，添加了查询的类别数
	...
	ATAQb = new double[l];
	ATe = new double[l];
#pragma omp parallel for private(i,j,k)		//并行处理
	for (i=0;i<nr_query;i++)	//不同的查询
	{
		k=1;
		for (j=0;j<count[i];j++)
		{
